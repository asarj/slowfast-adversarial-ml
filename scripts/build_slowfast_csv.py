...
# Classes were obtained from this link: https://github.com/facebookresearch/video-nonlocal-net/blob/master/process_data/kinetics/classids.json
...
def main(
    ...
    video_filenames_only = [os.path.basename(f) for f in videos_in_dir]
    
    # Create new dataframe to store the csv info
    final_df = pd.DataFrame(columns=["path", "class"])
    
    print("Begin data cleaning...")
    for i in range(len(video_filenames_only)):
        video = video_filenames_only[i]
    ...
    #     print(youtube_id, timestamps, ext)
        ...
        final_df = final_df.append({
            "path": "../" + videos_in_dir[i].as_posix().replace('"', ""),
            "class": label_id
        }, ignore_index=True)
